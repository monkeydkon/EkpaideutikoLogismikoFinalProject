...
public class ClassesTestFirstActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_classes_test_first);
        ...
        mDatabase.child("users").child(username).child("classesProgress").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if((Boolean)dataSnapshot.child("oopfalse").getValue()){
                    extra++;
                    if(extra == 1){
                        showMessage("Extra Question","C# is method oriented", false);
                    }
                    return;
...
//                }
        ...
        radioGroup.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                ...
                switch (checkedId){
                    case R.id.first:
                        mDatabase.child("questions").child("classes").child("oop").child("first").child("answers").child("correct").addValueEventListener(new ValueEventListener() {
                            @Override
                            public void onDataChange(DataSnapshot dataSnapshot) {
                                if (Integer.parseInt(dataSnapshot.getValue().toString()) != 1){
                                    ...
                                    oop=true;
                                    if(oop) //if you did a mistake on OOP
                                    {
                                        mDatabase.child("users").child(username).child("classesProgress").child("oopfalse").setValue(oop); //set value to true
                                    }
                                    //check();
                                }
                            }
                        ...
                        mDatabase.child("questions").child("classes").child("oop").child("first").child("answers").child("correct").addValueEventListener(new ValueEventListener() {
                            ...
                            public void onDataChange(DataSnapshot dataSnapshot) {
                                if (Integer.parseInt(dataSnapshot.getValue().toString()) != 2){
                                    ...
                                    if(oop)
                                    {
                                        mDatabase.child("users").child(username).child("classesProgress").child("oopfalse").setValue(oop);
                                    }
                ...
                Intent intent = new Intent(getApplicationContext(),ClassesTestSecondActivity.class);
